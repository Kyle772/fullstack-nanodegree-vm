...
def playerStandings():
    ...
    cur.execute("""
    SELECT p.ID, p.Name, coalesce(count(w.Winner), 0) as Wins,
           coalesce(count(w.Winner) + count(l.Loser), 0) AS TotalMatches
    FROM Players p
...
def reportMatch(winner, loser):
    ...
    """
    cur, conn = connect()
    print("Adding results of game data(Winner: {}, Loser:{})"
          .format(winner, loser))
